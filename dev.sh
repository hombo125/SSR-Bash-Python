...
if [[ $devc == 2 ]];then
	wget -q -N --no-check-certificate https://raw.githubusercontent.com/hombo125/shadowsock-install/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
...
if [[ ! -e bbr.sh ]]; then
	...
	if ! wget -N --no-check-certificate wget --no-check-certificate https://github.com/teddysun/across/raw/master/bbr.sh; then
...
if [[ $devc == 5 ]];then 
	[[ $OS = "CentOS" ]] && echo "本脚本仅支持 CentOS系统 BBR !" && exit 1
	echo "你要做什么？"
	echo "1.安装 BBR"
	echo "————————"
	echo "2.启动 BBR"
	...
	if [[ $ubbr == 1 ]];then
		...
		echo "2. 本脚本仅支持 centos 系统更换内核，OpenVZ和Docker 不支持更换内核"
		echo "3. centos 更换内核过程中会提示 [ 是否终止卸载内核 ] ，请选择 NO "
		echo ""
		echo -e "在下面输入\e[31;49m $rsum \e[0m表示您已知晓风险并同意安装，输入其它内容将退出安装！"
		read -n 6 -p "请输入： " choise
		if [[ $choise == $rsum ]];then
