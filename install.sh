...
if [[ -e ${libsodiumfilea} ]];then
    ...
    wget -q https://github.com/hombo125/shadowsocksr-backup/libsodium-1.0.16.tar.gz
    ...
    popd
    ldconfig
    cd $workdir && rm -rf libsodium-1.0.16.tar.gz libsodium-1.0.16
...
cd /usr/local
git clone https://github.com/hombo125/shadowsocksr
cd ./shadowsocksr
...
if [[ -d /usr/local/SSR-Bash-Python ]];then
    if [[ $yn == [yY] ]];then
        rm -rf /usr/local/SSR-Bash-Python
        cd /usr/local
        git clone https://github.com/hombo125/SSR-Bash-Python.git
    fi
...
if [[ $1 == "develop" ]];then
	wget -q -N --no-check-certificate -O /usr/local/bin/ssr https://raw.githubusercontent.com/hombo125/SSR-Bash-Python/master/ssr
