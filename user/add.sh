...
ssr_link_qr(){
	SSRprotocol=$(echo $ux1 | sed 's/_compatible//g')
	SSRobfs=$(echo $uo1 | sed 's/_compatible//g')
	SSRPWDbase64=$(urlsafe_base64 "$upass")
	SSRbase64=$(urlsafe_base64 "$myipname:$uport:${SSRprotocol}:$um1:${SSRobfs}:${SSRPWDbase64}")
